...
test('should log configuration with password obscurified upon successfull connection', async () => {
    const server = createServer()
    let logEntry
    server.events.once('log', entry => {
        logEntry = entry
    ...
    expect(logEntry).toEqual({
        ...
        tags: ['hapi-redis2', 'info'],
        data:
            'redis connection created for {"url":"redis://localhost:6379?password=******"}'
    })
    server.stop()
})

test('should be able to register plugin with URL and settings', async () => {
    const server = createServer()
    try {
        await server.register({
            plugin: require('./'),
